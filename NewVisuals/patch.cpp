...
SpatchST::SpatchST(const int &size_init, const int &x_init, const int &y_init, const int displayWidth, const int displayHeight, const int gridsize, array2D<double> &inN, array2D<double> &inS, array2D<double> &inE, array2D<double> &inW, double nuIn, bool isActiveInit)
{
    isActive = isActiveInit;
    x=x_init;
    y=y_init;
    ...
    if(isActive)
    {
        for(int i=x_init-size/2; i<x_init+size/2; ++i)
        {
            for(int j=y_init-size/2; j<y_init+size/2; ++j)
            {
                if(nuIn > drand())
                {
                    (*inNAddress)(i,(j+1)%gridsize)=(*inWAddress)(i,(j+1)%gridsize);
                    (*inSAddress)(i,j)=(*inWAddress)(i,j);
                }
                else
                {
                    (*inNAddress)(i,(j+1)%gridsize)=0;
                    (*inSAddress)(i,j)=0;
                }
            }
        }
...
void SpatchST::print (SDL_Surface* screen)
{
    if(isActive)
    {
        for(int i=x-size/2; i<=x+size/2; ++i)
...
void SpatchST::handleEvent(SDL_Event &event)
{
    if(!isActive)return;
    switch(event.type)
    {
