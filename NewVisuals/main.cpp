...
int main(int argc, char** argv)
{
    ...
    const bool STATICDEFECTS = true;
    const bool STATICVER = true;
    const bool INITPATCH = true;
    int psize=40; //Patch size
    double nuIn = 0.15;
    ...
    Spatch patch1 (psize, 50, 50, 400, 400, GRIDSIZE, inN, inS, inE, inW, nuIn, INITPATCH&&!STATICDEFECTS);
    Spatch patch2 (psize, 150, 150, 400, 400, GRIDSIZE, inN, inS, inE, inW, nuIn, INITPATCH&&!STATICDEFECTS);

    SpatchST patch3 (psize, 50, 50, 400, 400, GRIDSIZE, inN, inS, inE, inW, nuIn, INITPATCH&&STATICDEFECTS);
    SpatchST patch4 (psize, 150, 150, 400, 400, GRIDSIZE, inN, inS, inE, inW, nuIn, INITPATCH&&STATICDEFECTS);
    ...
    while(logic.getIsRunning())
    {
        ...
        while(SDL_PollEvent(&event))
        {
            ...
            patch3.handleEvent(event);
            patch4.handleEvent(event);
