...
void initialise_Kishan(std::vector< std::vector<double> > & link_N, std::vector< std::vector<double> > & link_E,std::vector< std::vector<double> > & link_S, std::vector< std::vector<double> > & link_W, int grid_size, double defect_density, double vert_conn, double firing_prob)
{
//    srand((unsigned int)time(NULL));
    for(int i=1; i<(grid_size+1); i++)
    {
        for(int j=0; j<grid_size; j++)
        {
            ...
            {
                link_W[i][j] = firing_prob;
                link_E[i][j] = firing_prob;
            }
            else
            {
                link_W[i][j] = 1.0;
    ...
    for(int i=1; i<(grid_size+1); i++)
    {
        for(int j=0; j<grid_size; j++)
        {
            ...
            {
                link_S[i][j] = link_W[i][j];
                link_N[i][(j+1)%grid_size] = link_W[i][(j+1)%grid_size];
            }
            else
            {
                link_S[i][j] = 0;
                link_N[i][(j+1)%grid_size] = 0;
            }
        }
    }
}
